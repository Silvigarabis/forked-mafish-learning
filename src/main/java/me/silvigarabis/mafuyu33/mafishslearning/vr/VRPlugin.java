...
import me.silvigarabis.mafuyu33.mafishslearning.config.CommonConstants;
...
public class VRPlugin {
    ...
    public static void initVR() {
        List<EntrypointContainer<IVRAPI>> apis = FabricLoader.getInstance().getEntrypointContainers("vrapi",
                IVRAPI.class);
        if (apis.size() > 0) {
            me.silvigarabis.mafuyu33.mafishslearning.vr.VRPlugin.getVRAPI(apis.get(0).getEntrypoint());
        }
    }
}
